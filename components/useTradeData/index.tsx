...
const useTradeData = () => {
    ...
    const fetchTrades = useCallback(async (isRealTime: boolean, dataType: string) => {
        setLoading(true);
        try {
            const response = await axios.get('https://10px.xyz/api/get-data', {
                params: {
            ...
            alert('当前获取数量过大，请耐心或分批获取。');
        } finally {
            setLoading(false);
        }
    }, [startTime, endTime, symbol, marketType, filterTrades, page, perPage]);

    const fetchLatestTrades = useCallback(async (dataType: string) => {
        setLoading(true);
        try {
            const response = await axios.get('https://10px.xyz/api/get-latest-orderbook', {
                params: {
                    market_type: marketType,
                    data_type: dataType,
                    symbol: symbol
    ...
    }, [marketType, filterTrades, symbol]);

    const exportTrades = async () => {
        try {
            const response = await axios.get('https://10px.xyz/api/export-data', {
                params: {
                    start_time: startTime,
                    end_time: endTime,
                    symbol: symbol,
    ...
    useEffect(() => {
        if (isRealTime) {
            const socket = new WebSocket('wss://10px.xyz/socket.io/?EIO=3&transport=websocket');
            socket.onopen = () => {
